...
const fmtAgo = date => {
  const now = Date.now();
  let d;
  if (typeof date === "number") d = date;else if (typeof date === "string") d = new Date(date).getTime();else if (date instanceof Date) d = date.getTime();else d = now;
...
window.__hh_load_data = async () => {
  ...
  const [meR, srvR, nodeR, usrR] = await Promise.allSettled([get("/api/me"), get("/api/servers"), get("/api/nodes"), get("/api/admin/users")]);
