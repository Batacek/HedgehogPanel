...
const VALID_PAGES = new Set([
  "home", "servers", "services", "nodes", "account",
  "admin.users", "admin.groups", "admin.perms",
  "admin.audit", "admin.plugins", "admin.settings",
]);

const parseHash = () => {
  let raw = (window.location.hash || "").replace(/^#/, "");
  try { raw = decodeURIComponent(raw); } catch (e) { /* keep raw on malformed % */ }
  if (raw.startsWith("server/")) {
    const id = raw.slice("server/".length);
    return id ? { page: "server-detail", serverId: id } : { page: "servers", serverId: null };
  }
  if (raw && raw !== "home" && VALID_PAGES.has(raw)) return { page: raw, serverId: null };
  return { page: "home", serverId: null };
};
...
const App = () => {
  ...
  React.useEffect(() => {
    window.__hh_load_data()
      .then(() => setDataLoaded(true))
      .catch(() => setDataLoaded(true)); // render with whatever loaded
  }, []);
  ...
  } : { uuid: "unknown", username: "user", firstname: "User", middlename: "", lastname: "", isAdmin: false });

  const user = { ...baseUser, ...profileEdits };
  ...
  React.useEffect(() => {
    ...
    if ((window.location.hash || "") === target) { firstHashSync.current = false; return; }
    const url = target || (window.location.pathname + window.location.search);
    if (firstHashSync.current) window.history.replaceState(null, "", url);
  ...
  const openServer    = (id) => { setServerId(id); setPage("server-detail"); };
  ...
  const refreshProps = { onRefresh: handleRefresh, refreshing, dataVersion };
    ...
    <>
      <div className="app" data-sidebar={sidebarMode}>
        ...
        <div style={{ minWidth: 0 }}>
          <Topbar crumbs={crumbs} user={user} onNav={nav} />

          {page === "home"          && <DashboardPage user={user} onNav={nav} onOpenServer={openServer} {...refreshProps} />}
          {page === "servers"       && <ServersPage onOpenServer={openServer} {...refreshProps} />}
          {page === "server-detail" && <ServerDetailPage serverId={serverId} onBack={() => nav("servers")} />}
          {page === "services"      && <ServicesPage onOpenServer={openServer} {...refreshProps} />}
          {page === "nodes"         && <NodesPage {...refreshProps} />}
          {page === "account"       && <AccountPage user={user} onUpdateUser={(p) => setProfileEdits(prev => ({ ...prev, ...p }))} />}

          {page === "admin.users"    && user.isAdmin && <AdminUsersPage />}
          {page === "admin.groups"   && user.isAdmin && <AdminGroupsPage />}
          {page === "admin.perms"    && user.isAdmin && <AdminPermissionsPage />}
          {page === "admin.audit"    && user.isAdmin && <AdminAuditPage />}
          {page === "admin.plugins"  && user.isAdmin && <AdminPluginsPage />}
          {page === "admin.settings" && user.isAdmin && (
            <AdminSettingsPage
              defaults={{ palette: tw.palette, lang }}
              onChangeDefaults={(d) => {
...
const TweaksUI = ({ tw, setTweak }) => {
  ...
  return (
    <TweaksPanel title="Hedgehog · Tweaks">
      <TweakSection label="Barevný směr">
        <TweakColor
          label="Akcent"
        ...
        />
        <div style={{ fontSize: 11.5, color: "#999", padding: "2px 12px 6px", textTransform: "capitalize" }}>
          {tw.palette}
        </div>
      </TweakSection>
